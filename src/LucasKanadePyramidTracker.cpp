...
LucasKanadePyramidTracker::LucasKanadePyramidTracker() :
        _windowSize(DEFAULT_WINDOW_SIZE), _maxLevel(
                DEFAULT_MAX_LEVEL), _flags(DEFAULT_FLAGS), _minEigenvalueThreshold(
                DEFAULT_MIN_EIGENVALUE_THRESHOLD), _maxErrorThreshold(
                DEFAULT_MAX_ERROR_THRESHOLD), _termCrit(
...
LucasKanadePyramidTracker::LucasKanadePyramidTracker(
        ...
        _windowSize(toCopy._windowSize), _maxLevel(toCopy._maxLevel), _flags(
                ...
                toCopy._maxErrorThreshold), _termCrit(
                toCopy._termCrit) {

}

LucasKanadePyramidTracker::LucasKanadePyramidTracker(
        const cv::Size &windowSize, const unsigned int &maxLevel,
        const int &flags, const cv::TermCriteria &terminationCriteria,
        ...
        _windowSize(windowSize), _maxLevel(maxLevel), _flags(flags), _minEigenvalueThreshold(
                minEigenvalueThreshold), _maxErrorThreshold(
                maxErrorValue), _termCrit(terminationCriteria) {
...
void LucasKanadePyramidTracker::trackFeatures(const cv::Mat &oldInput,
    ...
    cv::calcOpticalFlowPyrLK(oldInput, newInput, oldFeatures,
            newFeatures, status, err, _windowSize, _maxLevel,
            _termCrit, _flags, _minEigenvalueThreshold);
