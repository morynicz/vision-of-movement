...
bool compareListPoints(std::list<cv::Point2f> l1,
        std::list<cv::Point2f> l2) {
...
bool listPointsEqual(std::list<cv::Point2f> l1,
        std::list<cv::Point2f> l2) {
...
void FeatureExtractor::refillFeatures(const cv::Mat& oldFrame,
    ...
    if (features.size() > maxFeatures) {
        ...
        std::vector<cv::Point2f> additionalFeatures = extractFeatures(
                oldFrame, maxFeatures - oldFeatures.size());
        if (additionalFeatures.empty()) {
            cv::Exception ex(NO_FEATURES_FOUND, "no features found",
                    __func__, __FILE__, __LINE__);
            throw ex;
        }
        additionalFeatureList.resize(additionalFeatures.size());
        for (unsigned int i = 0; i < additionalFeatures.size(); ++i) {
            additionalFeatureList[i].push_front(
