...
ShiThomasiFeatureExtractor::ShiThomasiFeatureExtractor(
        ...
        _qualityLevel(toCopy._qualityLevel), _minDistance(
                toCopy._minDistance), _blockSize(toCopy._blockSize), _winSize(
                toCopy._winSize), _zeroZone(toCopy._zeroZone), _termCrit(
                toCopy._termCrit) {

}

ShiThomasiFeatureExtractor::ShiThomasiFeatureExtractor(
        const double &qualityLevel, const double &minDistance,
        const int &blockSize, const cv::Size &winSize,
...
std::vector<cv::Point2f> ShiThomasiFeatureExtractor::extractFeatures(
    ...
    if (!result.empty()) {
        cv::cornerSubPix(input, result, _winSize, _zeroZone,
                _termCrit);
    }
    return result;
}
