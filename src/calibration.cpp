...
static bool runAndSave(const string& outputFilename,
	...
	bool ok = runCalibration(imagePoints, imageSize, boardSize, patternType,
			squareSize, aspectRatio, flags, cameraMatrix, distCoeffs, rvecs,
...
int main(int argc, char** argv) {
	...
	for (i = 1; i < argc; i++) {
		...
		if (strcmp(s, "-w") == 0) {
			...
			if (sscanf(argv[++i], "%f", &cameraWidth) != 1 || cameraWidth <= 0)
		...
		if (!view.data) {
			if (imagePoints.size() > 0)
				runAndSave(outputFilename, imagePoints, imageSize, boardSize,
						pattern, squareSize, aspectRatio, flags, cameraMatrix,
