...
static void help() {
    printf(
            "This is a camera calibration sample.\n"
                    ...
                    "     [-cw <width>]            # set camera resolution width \n"
                    "     [-ch <width>]            # set camera resolution height \n"
                    "\n");
    printf("\n%s", usage);
    printf("\n%s", liveCaptureHelp);
}
...
static void saveCameraParams(const string& filename, Size imageSize,
        Size boardSize, float squareSize, float aspectRatio,
        int flags, const Mat& cameraMatrix, const Mat& distCoeffs,
        const vector<Mat>& rvecs, const vector<Mat>& tvecs,
...
static bool runAndSave(const string& outputFilename,
        ...
        Size boardSize, Pattern patternType, float squareSize,
    ...
    bool ok = runCalibration(imagePoints, imageSize, boardSize,
            patternType, squareSize, aspectRatio, flags, cameraMatrix,
    ...
    if (ok)
        saveCameraParams(outputFilename, imageSize, boardSize,
                squareSize, aspectRatio, flags, cameraMatrix,
                distCoeffs, writeExtrinsics ? rvecs : vector<Mat>(),
...
int main(int argc, char** argv) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (strcmp(s, "-w") == 0) {
            ...
            if (sscanf(argv[++i], "%u", &nframes) != 1
                    || nframes <= 3)
                return printf("Invalid number of images\n"), -1;
        } else if (strcmp(s, "-a") == 0) {
            if (sscanf(argv[++i], "%f", &aspectRatio) != 1
                    || aspectRatio <= 0)
            ...
            if (sscanf(argv[++i], "%f", &cameraWidth) != 1
                    || cameraWidth <= 0)
                return printf("Invalid cameraWidth\n"), -1;
            printf("cw %s %f\n", argv[i], cameraWidth);
        } else if (strcmp(s, "-ch") == 0) {
            if (sscanf(argv[++i], "%f", &cameraHeight) != 1
                    || cameraHeight <= 0)
                return printf("Invalid cameraHeight\n"), -1;
            printf("ch %s %f\n", argv[i], cameraHeight);
        } else if (s[0] != '-') {
            if (isdigit(s[0]))
                sscanf(s, "%d", &cameraId);
    ...
    if (capture.isOpened() && 0 != cameraWidth && 0 != cameraHeight) {
    ...
    for (i = 0;; i++) {
        ...
        if (found)
            drawChessboardCorners(view, boardSize, Mat(pointbuf),
                    found);
        ...
        if (mode == CAPTURING
                && imagePoints.size() >= (unsigned) nframes) {
            if (runAndSave(outputFilename, imagePoints, imageSize,
                    boardSize, pattern, squareSize, aspectRatio,
                    flags, cameraMatrix, distCoeffs, writeExtrinsics,
                    writePoints))
                mode = CALIBRATED;
            else
                mode = DETECTION;
