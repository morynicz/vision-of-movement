...
void refillFeatures(const cv::Mat& oldIn, vector<Point2f>& oldFeatures,
		unsigned maxCorners, double threshold,
		std::vector<std::list<Point2f> > &featureHistory);

void pyrLK(const cv::Mat &oldInput, const cv::Mat &newInput,
		vector<cv::Point2f>& oldFeatures, vector<cv::Point2f> &newFeatures,
		std::vector<std::list<Point2f> > &featureHistory);
...
//std::cerr<<imageBoardSize<<" "<<imageSize<<std::endl;
		...
		cv::Mat zeroBird;
		warpPerspective(greyNewIn(lowerRoi), zeroBird, perspTrans, greyNewIn.size(),
...
perspTrans = homN.clone();
	...
	do {
		...
		warpPerspective(newGround, newBirdGround, perspTrans, newGround.size(),
				INTER_LINEAR | WARP_INVERSE_MAP);
		warpPerspective(oldGround, oldBirdGround, perspTrans, oldGround.size(),
				INTER_LINEAR | WARP_INVERSE_MAP);
		showBird = newBirdGround.clone();

		refillFeatures(oldBirdGround, lowOldFeatures, maxCorners, 0.9,
				lowFeatureHistory);
		if (!lowOldFeatures.empty()) {
			pyrLK(oldBirdGround, newBirdGround, lowOldFeatures, lowNewFeatures,
					lowFeatureHistory);
			trimFeatureHistory(lowFeatureHistory, maxHistoryLength);
		}
		refillFeatures(oldFar, highOldFeatures, maxCorners, 0.9,
				highFeatureHistory);
		if (!highOldFeatures.empty()) {
			pyrLK(oldFar, newFar, highOldFeatures, highNewFeatures,
					highFeatureHistory);
			trimFeatureHistory(highFeatureHistory, maxHistoryLength);
		}

		vector<Point2f> trNewFeatures;
		vector<Point2f> trOldFeatures;
		...
		try {
			for (unsigned int i = 0; i < lowNewFeatures.size(); ++i) {
		...
		std::vector<float> rotations = getRotationAngles(lowOldFeatures,
				lowNewFeatures, 3.22,
				Point2f(greyNewIn.cols / 2, greyNewIn.rows / 2));
		...
		if (!translations.empty()) {
			...
			verTransHist = createTranslationHistogram(translations, 101, 480,
					true);
			//std::cerr << "dwa" << std::endl;
			horTransHist = createTranslationHistogram(translations, 101, 640,
					true);
		...
		//
		putText(shown, num2Str(lowNewFeatures.size()), Point(0, shown.rows),
...
void refillFeatures(const cv::Mat& oldIn, vector<Point2f>& oldFeatures,
		unsigned maxCorners, double threshold,
		std::vector<std::list<Point2f> >& featureHistory) {
	if (oldFeatures.size() > maxCorners) {
		oldFeatures.resize(maxCorners);
	} else if (oldFeatures.size() < threshold * maxCorners) {
		vector<Point2f> additionalFeatures;
		vector<std::list<Point2f> > additionalFeatureList;
		extractFeatures(oldIn, maxCorners - oldFeatures.size(),
				additionalFeatures);
